...
function getInfo2(){
    ...
    $ch = curl_init();
    $login_url = 'https://zhjw.neu.edu.cn/ACTIONQUERYSTUDENTSCORE.APPPROCESS';
    // 设置选项，包括URL
    //$header = array();
...
//echo $html;
/*$ql = QueryList::rules([
    'link' => ['form table tr td table tr td','text']
]);
//!!!大坑!!!head里面有中文，有乱码，需要先去除！
$data = $ql->setHtml($html)->removeHead()->query()->getData();*/

$rule = array(
    'link' => array('form table tr td table tr td','text')
);
//!!!大坑!!!head里面有中文，有乱码，需要先去除！
...
$gpa = $data[0]['link'];
$gpa = substr($gpa, -6, 6);
echo json_encode($gpa,JSON_UNESCAPED_UNICODE);
?>
