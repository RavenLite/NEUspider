...
require_once '02-03analyze.php';
$cookie = '';
$cookie_file = tempnam('./temp','cookie');
$stuID = $_GET["stuID"];
$stuPass = $_GET["stuPass"];
$YearTermNO = $_GET["YearTermNO"];

function getInfo(){
    global $cookie, $cookie_file;
    // 1. 初始化
    $ch = curl_init();
    $login_url = 'https://zhjw.neu.edu.cn/ACTIONLOGON.APPPROCESS?mode=1&applicant=ACTIONQUERYSTUDENTSCORE';
    // 2. 设置选项，包括URL
...
function sendPost(){
    ...
    $header = array(
        ...
        'Referer: https://zhjw.neu.edu.cn/ACTIONLOGON.APPPROCESS?mode=1&applicant=ACTIONQUERYSTUDENTSCORE',
...
function getInfo2(){
    ...
    $header = array(
        ...
        'Content-Type: application/x-www-form-urlencoded',
        'Host: zhjw.neu.edu.cn',
        'Referer: https://zhjw.neu.edu.cn/ACTIONQUERYSTUDENTSCORE.APPPROCESS',
        'Upgrade-Insecure-Requests: 1',
        'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/64.0.3282.186 Safari/537.36'
    );
    //设置body数据
    $post_data = array(
        'YearTermNO' => $YearTermNO
    );
    //需要一个转化
    $query = http_build_query($post_data);
    //设置cURL参数
    ...
    curl_setopt($ch, CURLOPT_HTTPHEADER, $header);//加入header
    curl_setopt($ch, CURLOPT_COOKIEFILE, $cookie_file);//插入cookie
    curl_setopt($ch, CURLOPT_URL, 'https://zhjw.neu.edu.cn/ACTIONQUERYSTUDENTSCORE.APPPROCESS');//设置目标地址
    curl_setopt($ch, CURLOPT_HEADER, 1);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_POST, true);//发送POST请求
    curl_setopt($ch, CURLINFO_HEADER_OUT, true);
